...
from agent.secret_store import save_secrets
from agent.setup_inputs import (
    AIProvider,
    InputFunction,
    OutputFunction,
)
...
from agent.setup_inputs import (
    valid_exposed_input as _valid_exposed_input,
)
from agent.setup_inputs import (
    valid_hostname as _valid_hostname,
)
from agent.setup_inputs import (
    valid_project_id as _valid_project_id,
)
from agent.setup_inputs import (
    yes_no as _yes_no,
)
...
class SetupAnswers:
    ...
    google_broad_access_acknowledged: bool = False
    ...
    microsoft_broad_access_acknowledged: bool = False
    ...
    def __post_init__(self) -> None:
        ...
        if self.google_enabled and not self.google_broad_access_acknowledged:
            raise ValueError("Google broad-access consent must be acknowledged")
        ...
        if self.microsoft_enabled and not self.microsoft_broad_access_acknowledged:
            raise ValueError("Microsoft broad-access consent must be acknowledged")
...
def generate_setup(
    ...
    *,
    secret_writer: Callable[[Mapping[str, str]], None] = save_secrets,
) -> SetupResult:
    ...
    artifacts, secret_values = _build_artifacts(
        answers,
        root,
        owner_token,
        project_path,
...
def _build_artifacts(
    ...
    sync_config = AppConfig.model_validate(
        {
            ...
            "google": {
                ...
                "acknowledge_broad_access": answers.google_broad_access_acknowledged,
                "write_mode": "replace" if answers.google_write_enabled else "read_only",
                "acknowledge_non_atomic_replacement": answers.google_write_enabled,
            },
            "microsoft": {
                ...
                "credential_keyring_user": answers.owner_user_id,
                "acknowledge_broad_access": answers.microsoft_broad_access_acknowledged,
            },
            "github": {
                "enabled": True,
    ...
    environment = "\n".join(
        (
            "# Generated by doccolab-setup. Secrets are stored in the OS keyring.",
...
def run_interactive(
    ...
    *,
    ...
    while True:
        google_enabled = _yes_no("Connect Google Docs?", False, input_fn)
        google_file_id = (
            _ask("Google Drive file ID", input_fn=input_fn) if google_enabled else None
        )
        google_broad_access_acknowledged = False
        google_write_enabled = False
        if google_enabled:
            output_fn(
                "Google's installed-app OAuth grants access beyond the configured file ID."
            )
            google_broad_access_acknowledged = _yes_no(
                "Acknowledge Google OAuth access scope?", False, input_fn
            )
            if not google_broad_access_acknowledged:
                output_fn("Google integration was disabled because consent was not acknowledged.")
                google_enabled = False
                google_file_id = None
            else:
                output_fn(
                    "Google full-document replacement is not an atomic compare-and-swap; "
                    "read-only mode is recommended."
                )
                google_write_enabled = _yes_no(
                    "Enable Google full-document replacement?", False, input_fn
                )
        microsoft_enabled = _yes_no("Connect Word Online / OneDrive?", False, input_fn)
        microsoft_item_id = (
        ...
        microsoft_broad_access_acknowledged = False
        if microsoft_enabled:
            output_fn(
                "Microsoft Files.ReadWrite.All grants access beyond the configured item ID."
            )
            microsoft_broad_access_acknowledged = _yes_no(
                "Acknowledge Microsoft Graph access scope?", False, input_fn
            )
            if not microsoft_broad_access_acknowledged:
                output_fn(
                    "Microsoft integration was disabled because consent was not acknowledged."
                )
                microsoft_enabled = False
                microsoft_item_id = None
                microsoft_drive_id = None
    ...
    output_fn("")
    output_fn("Secrets are stored in the operating system keyring.")
    ...
    answers = SetupAnswers(
        ...
        google_broad_access_acknowledged=google_broad_access_acknowledged,
        google_write_enabled=google_write_enabled,
        microsoft_broad_access_acknowledged=microsoft_broad_access_acknowledged,
    ...
    return generate_setup(answers, secret_writer=secret_writer)
