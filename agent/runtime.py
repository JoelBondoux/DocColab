...
def build_runtime(config: AppConfig) -> Runtime:
    ...
    google_drive = None
    google_docs = None
    uses_google = any(document.google_file_id for document in config.documents if document.enabled)
    ...
    return Runtime(manager=manager, state=state, github=github, graph=graph)
