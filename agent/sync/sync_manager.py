...
class SyncManager:
    ...
    async def run_forever(self) -> None:
        await self.sync_all()
        await asyncio.to_thread(self._initialize_microsoft_delta)
        while not self.stop_event.is_set():
            try:
                ...
                changed = await asyncio.to_thread(self._poll_microsoft_delta)
    ...
    def _sync_document(self, document: DocumentConfig) -> SyncResult:
        ...
        google_metadata = self._google_metadata(document)
        microsoft_metadata = self._microsoft_metadata(document)
        ...
        if google_changed:
            candidates[Source.GOOGLE] = self._google_markdown(document)
        if microsoft_changed:
            candidates[Source.MICROSOFT] = self._microsoft_markdown(document)
        ...
        if document.google_file_id and surface_hashes[Source.GOOGLE] != target_hash:
    ...
    def _publish_pending(
        ...
        if Source.GOOGLE.value in surfaces and google_metadata and document.google_file_id:
            ...
            if google_metadata.version != state.google_version:
                current = self._google_markdown(document)
                ...
                google_metadata = google_drive.replace_google_doc(
                    document.google_file_id,
                    docx,
        ...
        if (
            ...
            if microsoft_metadata.etag != state.microsoft_version:
                current = self._microsoft_markdown(document)
                if _content_hash(current) != target_hash:
                    conflict = self.conflicts.merge(
                        "",
                ...
                microsoft_metadata = onedrive.replace(
    ...
    def _google_metadata(self, document: DocumentConfig) -> GoogleFileMetadata | None:
        if not document.google_file_id:
            return None
        if not self.google_drive:
    ...
    def _microsoft_metadata(self, document: DocumentConfig) -> OneDriveMetadata | None:
    ...
    def _google_markdown(self, document: DocumentConfig) -> str:
    ...
    def _microsoft_markdown(self, document: DocumentConfig) -> str:
    ...
    def _initialize_microsoft_delta(self) -> None:
        if not self.onedrive:
            return
    ...
    def _poll_microsoft_delta(self) -> set[str]:
        if not self.onedrive:
            return set()
        changed: set[str] = set()
        for drive_id, documents in self._microsoft_groups().items():
            ...
            try:
                ...
                result = self.onedrive.poll_delta(drive_id=drive_id, latest_only=True)
            self.state.set_cursor("microsoft", scope, result.delta_link)
            item_ids = {str(item.get("id")) for item in result.items if "deleted" not in item}
            changed.update(
                document.id
