...
Provider adapters, webhook/auth ASGI boundaries, MCP tool dispatch, sync
transactions, runtime caching, and persistent delta/watch state are exercised with
fakes or in-process transports. Read-only live Google, Microsoft, and GitHub smoke
tests are opt-in through `DOCCOLAB_LIVE_CONFIG`.

- **Set up (Python):** install the `dev` extra and run `pytest`
- **Coverage SSOT:** `pyproject.toml` enables branch coverage and enforces 60%
- **Starter files:** `tests/test_sync_manager.py`, `tests/test_mcp_http.py`,
  `tests/test_provider_events.py`, `tests/test_live_provider_smoke.py`
