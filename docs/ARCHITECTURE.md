...
│   ├── sync/               State, merge, orchestration
...
Drive notification or metadata poll
  ...
  -> AI rewrite preserving Markdown structure
  -> optimistic AI commit
  -> Markdown to DOCX
  -> update the existing Drive file ID
...
- Google revision metadata is checked before replacement.
