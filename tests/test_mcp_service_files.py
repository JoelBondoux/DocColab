...
def test_file_tools_are_project_scoped_atomic_and_audited(tmp_path: Path) -> None:
    ...
    denied_identity = UserRecord(
        user_id="outsider@example.com",
        role=Role.EDITOR,
        token_sha256="b" * 64,
    )
    denied_context = set_current_user(denied_identity)
    try:
        with pytest.raises(PermissionError):
            service.write_file("alpha", "documents/blocked.md", "blocked")
    finally:
        reset_current_user(denied_context)
